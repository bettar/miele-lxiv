        ...
        if ([self autoFillComments])
        {
            ...
            if ([self commentsGroup] && [self commentsElement])
            {
                ...
                if (dicomItems->findAndGetString(key, string, OFFalse).good() && string != NULL)
                    commentsField = [DicomFile stringWithBytes: (char*) string encodings:myEncodings];
            }
            ...
            if ([self commentsGroup2] && [self commentsElement2])
            {
                ...
                {
                    if (commentsField)
                        commentsField = [commentsField stringByAppendingFormat: @" / %@", [DicomFile stringWithBytes: (char*) string encodings:myEncodings]];
                    else
                        commentsField = [DicomFile stringWithBytes: (char*) string encodings:myEncodings];
...
//			}  //ckeck LAVIN
    ...
    {
        name = [[DicomFile stringWithBytes: (char*) string encodings:myEncodings replaceBadCharacters:NO] retain];
        if (name == nil)
            name = [[NSString alloc] initWithCString: string encoding: myEncodings[ 0]];
    }
    else
...
//			if (manufacturer == nil) manufacturer = [[[NSString alloc] initWithCString: string encoding: myEncodings[0]] autorelease];
//			
...
//		}
    ...
    double orientationMultiFrame[ 9] = {1, 0, 0, 0, 1, 0};
